...
class ComplianceServiceTest(TestCase):
    ...
    def test_compliance_report_audio_only_warning(self):
        report = self.service.generate_compliance_report(
            patient_state="FL",
            provider_state="FL",
            patient_setting="home",
        ...
        audio_warnings = [w for w in report["warnings"] if "audio-only" in w.lower()]
        assert len(audio_warnings) > 0

    def test_compliance_report_prescribing_warning(self):
        report = self.service.generate_compliance_report(
            patient_state="FL",
            provider_state="NY",
            patient_setting="home",
        ...
        prescribing = [
            w for w in report["warnings"] if "prescribing" in w.lower()
        ]
        assert len(prescribing) > 0
